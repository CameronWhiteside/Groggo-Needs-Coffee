...
const ModeDescription = ({ activeControl }) => {
    ...
    const descriptions = {
        water: [`Groggo refuses to swim ever since that once traumatic lesson he had as a child, so water will destroy all edges among adjacent nodes.`],
        ...
        street: [`Streets connect nodes with an 80% time reduction. Groggo never turns at intersections; that's where most accidents take place, so he claims.`],
        ...
        shop: [`Overpriced, but it'll have to do. Oh, and Groggo finally worked up the courage to ask the barista on a date, and their shift ends in 10 minutes. So hurry.`],
